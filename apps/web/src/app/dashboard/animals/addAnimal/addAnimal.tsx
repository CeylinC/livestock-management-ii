...
export default function AddAnimal({ isOpen, setIsOpen, data }: IProp) {
  ...
  const handleSave = () => {
    if (data) {
      updateAnimal(
        new Animal({ ...form.getFieldsValue(), id: data.id }),
        user?.id!
      );
    } else {
      setAnimal(new Animal(form.getFieldsValue()), user?.id!);
    }
  };

  const handleDelete = data
    ? () => deleteAnimal(data.id, user?.id!)
    : undefined;
  ...
  return (
    <>
      <DrawerForm
        title={
          data ? "Mevcut Hayvan Kaydını Güncelle" : "Yeni Hayvan Kaydı Girişi"
        }
        isOpen={isOpen}
        setIsOpen={setIsOpen}
        formItems={formItems}
