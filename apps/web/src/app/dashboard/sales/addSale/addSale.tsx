...
export default function AddSale({ isOpen, setIsOpen, data }: IProp) {
  ...
  const handleSave = () => {
    if (data) {
      updateSale(new Sale({ ...form.getFieldsValue(), id: data.id }))
    } else {
      setSale(new Sale(form.getFieldsValue()))
    }
  };

  const handleDelete = data ? () => (deleteSale(data.id)) : undefined

  return (
    <>
      <DrawerForm
        title={data ? "Mevcut Satış Kaydını Güncelle" : "Yeni Satış Kaydı Girişi"}
        isOpen={isOpen}
        setIsOpen={setIsOpen}
        formItems={formItems}
        form={form}
