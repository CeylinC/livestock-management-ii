...
export default function AddBarnScreen() {
  const { selectBarn, selectedBarn, updateBarn, setBarn, deleteBarn } = useBarnStore();
  ...
  const onSubmit = () => {
    if (selectedBarn) {
      updateBarn(new Barn({ ...form.getFieldsValue(), id: selectedBarn.id, gender: form.getFieldValue('gender')[0] }))
    } else {
      setBarn(new Barn({ ...form.getFieldsValue(), gender: form.getFieldValue('gender')[0] }))
    }
    onNavigate();
  }
  ...
  return (
    <Provider>
      <Stack.Screen options={{ title: selectedBarn ? "Mevcut Ağıl Kaydını Güncelle" : "Yeni Ağıl Kaydı Oluştur" }} />
