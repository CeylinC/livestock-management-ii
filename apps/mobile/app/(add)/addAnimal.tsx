...
export default function AddAnimalScreen() {
  const { selectAnimal, selectedAnimal, updateAnimal, setAnimal, deleteAnimal } = useAnimalStore();
  ...
  const onSubmit = () => {
    if (selectedAnimal) {
      updateAnimal(new Animal({ ...form.getFieldsValue(), id: selectedAnimal.id, gender: form.getFieldValue('gender')[0] }))
    } else {
      setAnimal(new Animal({ ...form.getFieldsValue(), gender: form.getFieldValue('gender')[0] }))
    }
    onNavigate();
  }
  ...
  return (
    <Provider>
      <Stack.Screen options={{ title: selectedAnimal ? "Mevcut Hayvan Kaydını Güncelle" : "Yeni Hayvan Kaydı Oluştur" }} />
