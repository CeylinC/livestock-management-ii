...
export default function AddSaleScreen() {
   const { selectSale, selectedSale, updateSale, setSale, deleteSale } = useSaleStore();
    ...
    const onSubmit = () => {
      if (selectedSale) {
        updateSale(new Sale({ ...form.getFieldsValue(), id: selectedSale.id, category: form.getFieldValue('category')[0], paymentState: form.getFieldValue('paymentState')[0]}))
      } else {
        setSale(new Sale({ ...form.getFieldsValue(), gender: form.getFieldValue('gender')[0] }))
      }
      onNavigate();
    }
    ...
    return (
      <Provider>
        <Stack.Screen options={{ title: selectedSale ? "Mevcut Satışın Kaydını Güncelle" : "Yeni Satış Kaydı Oluştur" }} />
